...
void _judgeTable() {
  group('what one step turns out to be, without a workflow on disk', () {
    // The classification, asked directly. Every other test here writes a
    // workflow to a temporary directory to reach these six outcomes, which
    // makes the extraction and the judgement one thing to get wrong.
    const declared = {'check', 'web'};

    StepVerdict verdictFor(
      String command, {
      String? exemption,
      bool shared = false,
    }) => judge(
      CiStep(
        ...
        command,
        exemption: exemption,
        exemptionIsShared: shared,
      ),
      declared,
    );

    test('a step running a declared gate set is that invocation', () {
      final verdict = verdictFor('dart run :xtask check');
      expect(verdict.gate, 'check');
      expect(verdict.problems, isEmpty);
    });

    test(
      'a gate set the file does not declare is a problem, and not a run',
      () {
        final verdict = verdictFor('dart run :xtask chekc');
        expect(verdict.gate, isNull);
        expect(verdict.problems.single, isA<RunsAnUndeclaredGate>());
      },
    );

    test('a step asking xtask a question is a question', () {
      expect(verdictFor('dart run :xtask --validate').question, '--validate');
    });

    test('anything else is a command that belongs in the task file', () {
      expect(verdictFor('npm ci').problems.single, isA<RunsACommand>());
    });

    test('a marker with a reason excuses it', () {
      final verdict = verdictFor('npm ci', exemption: 'the browser driver');
      expect(verdict.exempted, isTrue);
      expect(verdict.problems, isEmpty);
    });

    test('a marker with no reason excuses nothing and is reported', () {
      final verdict = verdictFor('npm ci', exemption: '');
      expect(verdict.exempted, isFalse);
      expect(verdict.problems.map((p) => p.runtimeType), [
        ExemptsWithoutSaying,
        RunsACommand,
      ]);
    });

    test('a marker over a gate set is reported as excusing nothing', () {
      final verdict = verdictFor('dart run :xtask check', exemption: 'why');
      expect(verdict.gate, isNull);
      expect(verdict.problems.single, isA<ExemptsNothing>());
    });

    test('but one shared with other commands leaves the gate a run', () {
      // A marker written for a whole line or block is necessarily idle over
      // some of what it covers, and saying so would make it unusable there.
