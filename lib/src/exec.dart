...
/// happened and touches neither the budget nor the exclusive tokens, so it can
/// be checked as a table rather than by running a plan. Those two are the
/// caller's, because taking a place or a token is not a decision but an act.
///
/// A step may begin when everything it waits on has FINISHED, which is why the
/// plan's order decides only which of the ready ones begins first: §4.3's
/// cheap-before-slow is a preference here rather than a guarantee.
Admission admits(
  PlanStep step, {
  required Set<String> finished,
  required Set<String> stopped,
  required bool anythingFailed,
  ...
  // Something has failed and this run is not keeping going: what has not
  // started must not start. What IS running is left alone.
  if (anythingFailed && !keepGoing) {
...
final class Executor {
  ...
  ///
  /// Off by default: §5.2 promises the run stops at the first failure, and a
  /// pipeline wants the earliest red. A person fixing things locally wants the
  /// whole list instead, which is §8's argument for `--validate` collecting
  /// every problem rather than costing a round of fix-and-rerun each.
  ...
  ///
  /// Above 1, §5.2's live output is deliberately given up: two tasks writing
  /// to one terminal produce a transcript belonging to neither, and a §7.1
  /// section folding lines from two tasks folds nothing — so each task's
  /// output is collected and printed whole when it finishes. Sequential and
  /// watchable, or parallel and buffered; the caller says which.
  ///
  /// §4.3's declaration order survives as a preference: it decides which of
  /// the ready tasks starts first, not which finishes.
  ...
  Future<int> run(Plan plan) async {
    ...
    // Whether two things could be in flight at once — which is what buffering
    // is for, and what the announcement below is about. Asked once and handed
    // down, so the two cannot disagree. A plan of one task cannot have two,
    // and a fanned-out task can: whether it is one is [Fanout.couldOverlap],
    // the same question the fan-out asks a member count later.
    final concurrent =
  ...
  ///
  /// One loop for both `-j 1` and `-j n`, since at one task in flight the
  ...
  /// goes first, takes the places and the tokens, and collects the outcomes.
  ///
  ...
  Future<int> _walk(
    ...
    // tasks finish in depends on the machine; the plan's order does not.
    final failures = <int, int>{};

    while (waiting.isNotEmpty || running.isNotEmpty) {
      var began = false;
      for (var at = 0; at < waiting.length; at++) {
        // The gate that keeps a sequential run sequential: at one task in
        // flight a failure is observed before the next is admitted, which is
        // what "a failure stops what has not started" rests on. The budget
        // below it is spent by UNITS, so `-j 4` over one fanned-out task lets
        // its members have all four places.
        ...
        final verdict = admits(
          ...
          finished: finished,
          stopped: stopped,
          anythingFailed: failures.isNotEmpty,
          keepGoing: keepGoing,
        );
        if (verdict case SkipIt(:final why)) {
          skipped[step.task.name] = why;
        ...
        // Left where it is rather than admitted and blocked: an admitted task
        // holds one of `-j`'s places whether or not it is doing anything, so
        // three tasks sharing a browser at `-j 3` would keep every independent
        // task out of the run.
    ...
    // order decides among failures of the same kind. Position alone is not
    // enough: a continuation always sits before the unrelated tasks after its
    // origin, and 4 says "only a `then:` failed" — a claim no run where an
    // ordinary task also failed may make.
  ...
  Future<int?> _runOne(
    ...
    // Waited for before the clock starts, so a task queued behind another
    // task's members is not billed for the wait. Taken here and HANDED ON to
    // the first member, which gives it back when it ends: releasing it here
    // would put the body back in the queue, and holding it for the task's
    // whole life deadlocks two two-member tasks at `-j 2`.
