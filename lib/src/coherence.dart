...
///
/// Not the parser's: `parse.dart` checks the document against the types, and
/// whether `each:` and `$each` agree is the same kind of question as a
/// dangling `needs:`.
///
/// Answered with a list rather than a throw, so one refusal names every
/// contradiction in a task instead of costing a round of fix-and-rerun each.
/// `parse.dart` is still the gate that refuses on it.
library;
...
///
/// A `run:` body only, for both keys. Dart cannot stop a function from
/// outside, so a `timeout:` on a verb would pass while it went on writing and
/// `interruptible:` would be a promise nothing keeps. A composite has no body
/// at all — its `needs:` are their own tasks and answer for themselves.
