...
///
/// How much of `-j` a fanned-out task may spend, whether its members' output
/// is collected, which member's failure the run answers with, what the others
/// did afterwards, and how much work there was altogether. All of it is about
/// the members of one task and none of it about the plan.
///
/// [Fanout.couldOverlap] is "could two things be writing at once", asked once:
/// the run asks it of a task before the set is read, and this asks it again
/// with the member count in hand.
library;
...
final class Fanout {
  ...
  ///
  /// Named by a key rather than a count, because it is asked before the set is
  /// read: the announcement has to come first, or it explains a silence that
  /// has already happened. `each:` without `serial:` is the shape that can fan
  /// out, and that is readable from the file with no filesystem near it.
  ///
  /// The whole of the question, in one place — the announcement, whether a
  /// task's output is collected, and whether its members run together all
  /// depend on it.
  ...
  Future<FanoutOutcome> run(
    ...
    // Keyed by index, so which failure the run answers with does not depend on
    // which member finished first — a `do:` verb's exit code is a deliberate
    // decision, and two members answering 2 and 1 would otherwise make the
