...
///
/// What `--dry-run` prints under a `do: remove` block, so that the one verb
/// which deletes recursively can be checked against what its author meant.
///
/// It does not call the verb: it asks what the verb asks, and removes nothing.
///
/// Two answers rather than one empty list. `--dry-run` renders an empty list
/// as "nothing of these is on disk, which is not an error", so anything the
/// run would refuse has to say so itself — `refused` carries the run's own
/// sentence and `paths` is only ever what would go.
